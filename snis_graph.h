...
extern void sng_set_scale(int wn, float xscale, float yscale);

extern void sng_current_draw_line(int wn, float x1, float y1, float x2, float y2);
extern void sng_current_draw_thick_line(int wn, float x1, float y1, float x2, float y2);
extern void sng_current_draw_rectangle(int wn, int filled, float x, float y, float width, float height);
...
SNG_GLOBAL void sng_draw_vect_obj(int wn, struct my_vect_obj *v, float x, float y);
SNG_GLOBAL float sng_abs_xy_draw_letter(int wn, struct my_vect_obj **font,
		unsigned char letter, float x, float y);
SNG_GLOBAL void sng_abs_xy_draw_string(int wn, char *s, int font, float x, float y);
SNG_GLOBAL void sng_abs_xy_draw_string_with_cursor(int wn,
				char *s, int font, float x, float y, int cursor_pos, int cursor_on) ;
SNG_GLOBAL void sng_center_xy_draw_string(int wn, char *s, int font, float x, float y);
SNG_GLOBAL void sng_center_xz_draw_string(int wn, char *s, int font, float x, float y);
SNG_GLOBAL void sng_string_bounding_box(char *s, int font, float *bbx1, float *bby1, float *bbx2, float *bby2);
SNG_GLOBAL void sng_draw_point(int wn, float x, float y);
...
SNG_GLOBAL void sng_draw_laser_line(int wn, float x1, float y1, float x2, float y2, int color);

/* pixel size of the actual output window */
SNG_GLOBAL void sng_set_screen_size(int wn, int width, int height);

/* size of the extent given to primitive drawing functions */
SNG_GLOBAL void sng_set_extent_size(int wn, int width, int height);

/* clip inside the extent */
SNG_GLOBAL void sng_set_clip_window(int wn, int x1, int y1, int x2, int y2);

/* offet and scale 3d rendering inside extent bounds */
SNG_GLOBAL void sng_set_3d_viewport(int wn, int x_offset, int y_offset, int width, int height);
...
SNG_GLOBAL float sng_pixelx_to_screenx(int wn, float x);
SNG_GLOBAL float sng_pixely_to_screeny(int wn, float y);

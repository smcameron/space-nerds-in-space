...
static int disable_terminal_rebooting = 0; /* tweakable, 1 disables terminal rebooting */
...
static void delete_object(struct snis_entity *o)
{
	...
	/* Omit OBJTYPE_NPCSHIP here because we want the registration preserved for the derelict */
...
static void distribute_damage_to_damcon_system_parts(struct damcon_data *d, int damage, int damcon_system)
{
	...
	if (!d) /* OBJTYPE_NPCSHIP don't have all the systems OBJTYPE_BRIDGE have */
		return;
...
static void reboot_random_terminals(struct snis_entity *player_ship)
{
	const uint8_t reboot = OPCODE_TERMINAL_EFFECT_REBOOT;

	if (disable_terminal_rebooting)
		return;
...
static void buddy_join_the_fight(uint32_t id, struct snis_entity *attacker, int recursion_level)
{
	...
	if (!o->alive)
		return;
	if (o->type != OBJTYPE_NPCSHIP) /* shouldn't happen */
		return;
...
static void push_attack_mode(struct snis_entity *attacker, uint32_t victim_id, int recursion_level)
{
	...
	if (attacker->type != OBJTYPE_NPCSHIP) {
	...
	if (go[i].type == OBJTYPE_NPCSHIP || go[i].type == OBJTYPE_BRIDGE) {
		...
		if (attacker->tsd.ship.in_secure_area || go[i].tsd.ship.in_secure_area) {
			...
			if (attacker->type != OBJTYPE_NPCSHIP || attacker->tsd.ship.ai[0].ai_mode != AI_MODE_COP)
...
static void attack_your_attacker(struct snis_entity *attackee, struct snis_entity *attacker)
{
	...
	if (attackee->type != OBJTYPE_NPCSHIP)
		return;
...
static void process_potential_victim(void *context, void *entity)
{
	...
	if (v->type != OBJTYPE_STARBASE && v->type != OBJTYPE_BRIDGE &&
		v->type != OBJTYPE_NPCSHIP && v->type != OBJTYPE_SPACEMONSTER)
		return;
...
static void pop_ai_attack_mode(struct snis_entity *o)
{
	...
	if (o->type != OBJTYPE_NPCSHIP) /* Could be a starbase, calling in here from torpedo collision detection */
		return;
...
static void notify_a_cop(void *context, void *entity)
{
	...
	if (cop->type != OBJTYPE_NPCSHIP)
		return;

	if (perp->type == OBJTYPE_STARBASE)
		return;
	
	if (perp->type == OBJTYPE_NPCSHIP && perp->tsd.ship.ai[0].ai_mode == AI_MODE_COP)
		return;
...
static void notify_the_cops(struct snis_entity *weapon, struct snis_entity *target)
{
	...
	if (target->type == OBJTYPE_NPCSHIP && target->tsd.ship.ai[0].ai_mode == AI_MODE_COP) {
	...
	if (dist2 < flare_proximity_distance * flare_proximity_distance && target->type == OBJTYPE_FLARE) {
		if (snis_randn(1000) < 1000.0 * flare_confuse_chance) {
			...
			if (i >= 0) {
				...
				if (o->alive && o->type == OBJTYPE_NPCSHIP) {
...
static void torpedo_collision_detection(void *context, void *entity)
{
	...
	if (o->alive >= torpedo_lifetime - 3)
		return;
	if (t->type != OBJTYPE_BRIDGE && t->type != OBJTYPE_NPCSHIP &&
...
static void ai_atk_near_enemy_mode_brain(struct snis_entity *o)
{
	...
	if (i < 0)
		return;
	e = &go[i];
	if (!e->alive || e->type != OBJTYPE_NPCSHIP || e->sdata.faction == o->sdata.faction) {
...
static void ai_rts_guard_base(struct snis_entity *o)
{
	...
	for (i = 0; i <= snis_object_pool_highest_object(pool); i++) {
		...
		if (!s->alive)
			continue;
		if (s->type != OBJTYPE_NPCSHIP) /* What about human controlled ships here? */
			continue;
...
static void player_collision_detection(void *player, void *object)
{
	...
	if (t->type == OBJTYPE_NPCSHIP && t->tsd.ship.shiptype == SHIP_CLASS_ASTEROIDMINER) {
	...
	if (t->type == OBJTYPE_NPCSHIP && t->tsd.ship.shiptype == SHIP_CLASS_MANTIS) {
		return;
	}
...
static void starbase_move(struct snis_entity *o)
{
	...
	for (i = 0; i < o->tsd.starbase.nattackers; i++) {
		...
		if (!a->alive)
			continue;
		if (a->type != OBJTYPE_BRIDGE && a->type != OBJTYPE_NPCSHIP)
			continue;
...
static int add_ship(int faction, int shiptype, int auto_respawn)
{
	...
	i = add_generic_object(x, y, z, 0.0, 0.0, 0.0, heading, OBJTYPE_NPCSHIP);
	if (i < 0)
		return i;
	go[i].move = ship_move;
...
static int add_asteroid(double x, double y, double z, double vx, double vz, double heading, double velocity)
{
	...
	go[i].orientation = random_orientation[go[i].id % NRANDOM_ORIENTATIONS];
					...
					char *npcname, struct npc_bot_state *botstate)
{
	...
	for (i = 0; i < ship_registry.nentries; i++) {
		...
		case SHIP_REG_TYPE_BOUNTY:
			...
			if (ship < 0)
				break;
			if (go[ship].type != OBJTYPE_NPCSHIP && go[ship].type != OBJTYPE_DERELICT)
				break;
					...
					uint32_t starbase_dispatcher)
{
	if (!tow_ship)
		return;
	if (tow_ship->type != OBJTYPE_NPCSHIP)
		return;
...
#define MAX_SHIPS_HAILABLE 10
	...
	for (i = 0; i <= snis_object_pool_highest_object(pool); i++) {
		...
		if (o->type != OBJTYPE_STARBASE && o->type != OBJTYPE_NPCSHIP)
			continue;
		if (o->type == OBJTYPE_NPCSHIP && o->tsd.ship.ai[0].ai_mode != AI_MODE_MINING_BOT)
			continue;
		sbname = o->sdata.name;
		for (j = 0; j < nnames; j++) {
...
static int l_set_faction(lua_State *l)
{
	...
	if (go[i].type == OBJTYPE_NPCSHIP) /* clear ship ai stack */
		go[i].tsd.ship.nai_entries = 0;
	go[i].timestamp = universe_timestamp;
	lua_pushnumber(l, 0.0);
	return 1;
...
static int l_set_commodity_contents(lua_State *l)
{
	...
	if (i > snis_object_pool_highest_object(pool))
		goto out;
	o = &go[i];
	if (o->type != OBJTYPE_BRIDGE && o->type != OBJTYPE_CARGO_CONTAINER && o->type != OBJTYPE_NPCSHIP)
		goto out;
...
static void rts_ai_assign_orders_to_units(int starbase_count, int unit_count)
{
	...
	for (i = 0; i <= snis_object_pool_highest_object(pool); i++) {
		struct snis_entity *o = &go[i];
		if (o->alive && o->type == OBJTYPE_NPCSHIP && o->sdata.faction == rts_ai.faction) {
			int unit_type = ship_type[o->tsd.ship.shiptype].rts_unit_type;
	...
	char *name)
{
	...
	if (dest->type == OBJTYPE_PLANET)
		...
		modifier = "the asteroid ";
	else if (dest->type == OBJTYPE_BRIDGE || dest->type == OBJTYPE_NPCSHIP)
		modifier = "the ship ";
	else
		modifier = "";

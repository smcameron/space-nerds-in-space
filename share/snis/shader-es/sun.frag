 ...
 *
 * Kept structurally identical to share/snis/shader/sun.frag so the two stay easy to diff;
 * only the dialect differs (varying/gl_FragColor).
 */
void main()
{
	...
	gl_FragColor = clamp(filmic_tonemap(vec4(max(emission, vec3(0.0)), coverage)), 0.0, 1.0);
}
